...
type ButtonSize =
	'w78h42' | 'w80h40' | 'w104h48' | 'w113h42' | 'w119h40' | 'w139h40' | 'w147h40' | 'w155h46' | 'w225h58' |
	'w239h63' | 'w320h44' | 'w369h48' | 'w376h58' | 'w382h48' | 'w398h52' | 'w579h56' | 'w735h52' | 'w1086h56' | 'w1482h48';
type ButtonColor = 'primary' | 'secondary' |'tertiary'| 'quaternary'| 'quinary'| 'danger' | 'success'| 'kakao'| 'disabled';
...
const colorStyles: Record<ButtonColor, string> = {
  ...
  tertiary: 'bg-[var(--color-gray-white)] text-[var(--color-black-secondary)] border border-[var(--color-gray-light)]',
  quaternary: 'bg-[var(--color-white)] text-[var(--color-purple-primary)] border border-[var(--color-purple-primary)]',
  quinary: 'bg-[var(--color-white)] text-[var(--color-black-secondary)] border border-[var(--color-gray-light)]',
  danger: 'bg-[var(--color-danger)] text-white',
  success: 'bg-[var(--color-success)] text-white',
  kakao: 'bg-[var(--color-kakao)] text-[var(--color-black-primary)]',
  disabled: 'bg-[var(--color-gray-light)] text-[var(--color-gray-secondary)] disabled:cursor-not-allowed',
};
...
const Button: React.FC<ButtonProps> = ({
	...
	const { width, height } = sizeMap[size as ButtonSize] ?? { width: '', height: '' };
	...
	const style = (!disabled && color === 'primary') ? { backgroundImage: 'var(--color-main)' } : undefined;
	return (
		<button
			type={type}
			onClick={onClick}
